...
/* harmony export */   "mz": () => (/* binding */ validateBranchName),
/* harmony export */   "EN": () => (/* binding */ validateEncoding),
/* harmony export */   "V6": () => (/* binding */ validateOutputLength)
/* harmony export */ });
...
const validateBranchName = (branchName) => {
    if (!branchName) {
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed("Can't obtain the branch from the environment. Please try again.");
    }
    if (!branchName?.length) {
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed('Branch name is empty please provide a branch with at least one character. Please try again.');
    }
    return branchName;
};
const validateEncoding = (inputEncoding) => {
    if (!['hex', 'base64'].includes(inputEncoding)) {
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed('The input encoding you entered is not valid. Please provide either "hex" or "base64"');
    }
    return inputEncoding;
};
const validateOutputLength = (inputOutputLength) => {
    if (inputOutputLength !== 'none') {
        let validatedInput;
        validatedInput = parseInt(inputOutputLength);
        if (validatedInput === 0) {
            _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed('The desired ouput length must be at least 1');
        }
...
/* harmony import */ var _actions_core__WEBPACK_IMPORTED_MODULE_0__ = __nccwpck_require__(186);
/* harmony import */ var _actions_core__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__nccwpck_require__.n(_actions_core__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _helper__WEBPACK_IMPORTED_MODULE_1__ = __nccwpck_require__(947);

const { createHash } = await Promise.resolve(/* import() */).then(__nccwpck_require__.t.bind(__nccwpck_require__, 5, 23));

async function run() {
    try {
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.debug(process.env.GITHUB_HEAD_REF || 'No head ref available');
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.debug(process.env.GITHUB_REF || 'No ref available');
        _actions_core__WEBPACK_IMPORTED_MODULE_0__.debug(process.env.GITHUB_REF_NAME || 'no ref name available');
        const branchName = (0,_helper__WEBPACK_IMPORTED_MODULE_1__/* .validateBranchName */ .mz)(process.env.GITHUB_HEAD_REF);
